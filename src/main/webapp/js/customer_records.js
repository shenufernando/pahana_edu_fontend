    ...
    customers.forEach(customer => {
        ...
        row.innerHTML = `
            ...
            <td class="action-links">
                ...
                <a href="#" class="delete-link" onclick="deleteCustomer(${customer.accountNo}, '${customer.name}')">Delete</a>
            </td>
        `;
...
function deleteCustomer(accountNo, customerName) {
    if (!confirm(`Are you sure you want to delete customer: ${customerName} (Account: ${accountNo})?`)) {
        return false;
    }
    
    console.log("Deleting customer:", accountNo);
    
    fetch(`http://localhost:8080/Pahana_edu_backend2/DeleteCustomerServlet?accountNo=${accountNo}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
        }
